/* AMOVE.f -- translated by f2c (version 20000121).
...
#define ab ((real *)&equiv_8)
    ...
    /* Local variables */
    static doublereal ans, tneg[3];
    static integer ipos;
    extern /* Subroutine */ int avadd_();
    static integer ire;
    extern /* Subroutine */ int aerr_(), amind_(), avdot_(), avsub_(), avsto_(
	    ), avleng_(), adyndp_();
    static integer icount;
    extern /* Subroutine */ int avmult_();
    static doublereal ftrmov;
    extern /* Subroutine */ int atltst_();
...
/* ... */
/* ... */
/* ...    THIS SUBROUTINE IS TO TAKE THE CUTTER FROM THE TE LOCATION, */
/*      WHICH IS ASSUMED TO BE EXACTLY ON THE IS SUBSCRIPTED */
/*      SURFACE, AND MOVE IT IN THE DIRECTION GDIR, EXACTLY, UNTIL */
/*      IS OFFSET FROM THE SURFACE */


    adyndp_(ab, &c__9, &c_b3);
/* ... */
/* ... IPOS = 0 ... NO POSITIVE S ENCOUNTERED YET */
/*           OTHERWISE ... POSITIVE S ENCOUNTERED AND HOW MANY TIMES */
/* ... */
    ipos = 0;
    icount = 0;
/* ... FIRST TIME IN, CUTTER IS MOVED IN GDIR DIRECTION */
/*    BY FTRMOV = CR * .5 * SQRT(1 - ((TA . GDIR) ** 2)) */
/*    IN SUBSEQUENT TIMES WHEN +S, FTRMOV  * .5 */
    avdot_(a2cmmn_1.ta, &gdir[1], &ans);
    ans = a2fpc_1.q1p - ans * ans;
    ans = sqrt(ans);
    ftrmov = a2cmmn_1.cutdat[0] * ans * a2fpc_1.qp5;
    avmult_(&gdir[1], &ftrmov, a2cmmn_1.vtem);
...
/* ... */
    ...
    avsub_(a2cmmn_1.vnul, &gdir[1], &a2cmmn_1.tn[a2cmmn_1.is - 1]);
    avsto_(a2cmmn_1.te, &a2cmmn_1.tp[a2cmmn_1.is - 1]);
    a2cmmn_1.jent[a2cmmn_1.iis - 1] = 1;
    a2cmmn_1.iafl[a2cmmn_1.iis - 1] = -1;
...
/* ... */
    avsto_(a2cmmn_1.te, tneg);
    if (icount > 20 && ipos == 0) {
	goto L30;
    }
/* ... */
    avmult_(&gdir[1], &a2cmmn_1.s[a2cmmn_1.is - 1], a2cmmn_1.vtem);
    avsub_(a2cmmn_1.te, a2cmmn_1.vtem, a2cmmn_1.te);
    goto L10;
L30:
/* ... */
    ans = a2fpc_1.q5p * a2cmmn_1.s[a2cmmn_1.is - 1];
    avmult_(&gdir[1], &ans, a2cmmn_1.vtem);
    avsub_(a2cmmn_1.te, a2cmmn_1.vtem, a2cmmn_1.te);
    goto L10;
...
/* ... */
    ...
    goto L10;
L25:
    avsub_(a2cmmn_1.te, tneg, a2cmmn_1.vtem);
