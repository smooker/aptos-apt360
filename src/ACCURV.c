/* ACCURV.f -- translated by f2c (version 20000121).
...
/* Subroutine */ int accurv_(issto, iissto, sp3, spl3, sn3)
integer *issto, *iissto;
doublereal *sp3, *spl3, *sn3;
{
    /* Initialized data */
	...
	char e_1[8];
	real e_2;
	} equiv_8 = { {'A', 'C', 'C', 'U', 'R', 'V', ' ', ' '}, (float)0. };

#define ab ((real *)&equiv_8)
    ...
    /* Local variables */
    static integer i__, iq;
    static doublereal p00001, ans;
    static integer jjw;
    static doublereal qqq45p;
    extern /* Subroutine */ int avdot_(), astor_(), avsto_(), avsub_(), 
	    adyndp_(), avnorm_(), avmult_();
...
/* *** */
/* *** */
    p00001 = a2fpc_1.qp001;
    qqq45p = a2fpc_1.q45p;
    if (a2cmmn_1.j24[a2cmmn_1.iis - 1] == 7 || a2cmmn_1.j24[a2cmmn_1.iis - 1] 
	    == 8) {
	goto L400;
    }
    if (a2cmmn_1.jam == 1 || a2cmmn_1.ifrl == 1) {
	goto L400;
    }
    goto L500;
L400:
    p00001 = a2fpc_1.qp4x01;
    qqq45p = a2fpc_1.q4500p;
L500:
/* .................. P00001 = QP001 OR QP4X01 */
/* .................. QQQ45P = Q45P OR Q4500P */
/* *** */
    adyndp_(ab, &c__5, &c_b5);
/* ..   IQ = IPONTR(IISSTO) */
/* ..   CALL ASTOR(CANSTO(IQ),8,JJW,4) */
/* ..   IF((JJW.EQ.4).OR.(JJW.EQ.5)) GO TO 300 */
/* ..   IF((JJW.EQ.8).OR.(JJW.EQ.13)) GO TO 300 */
    if (a2cmmn_1.ipl[*iissto - 1] != 0) {
	goto L9;
    } else {
	goto L1;
    }
L9:
    a2cmmn_1.isigcr[*iissto - 1] = -1;
    goto L6;
L1:
    a2cmmn_1.n = 1;
    for (i__ = 1; i__ <= 3; ++i__) {
	a2cmmn_1.temp[i__ - 1] = sp3[i__] - spl3[i__];
	if ((d__1 = a2cmmn_1.temp[i__ - 1], abs(d__1)) <= p00001) {
	    goto L2;
	}
/* L11: */
	a2cmmn_1.n = 0;
...
L3:
    avnorm_(a2cmmn_1.temp, &a2cmmn_1.temp[3], &a2cmmn_1.ier);
    goto L21;
L10:
    if (a2cmmn_1.j24[*iissto - 1] <= 5 && abs(ans) >= a2fpc_1.q15x01) {
	goto L4;
    }
    a2cmmn_1.a = a2cmmn_1.rc[*issto - 1];
    a2cmmn_1.rc[*issto - 1] = qqq45p;
...
L20:
    ...
    goto L6;
L21:
    avdot_(&sn3[1], &a2cmmn_1.temp[3], &ans);
    if (abs(ans) < p00001) {
	goto L10;
    }
L4:
    avdot_(a2cmmn_1.temp, &sn3[1], &a2cmmn_1.rc[*issto - 1]);
/* L5: */
    avdot_(a2cmmn_1.temp, a2cmmn_1.temp, &ans);
    a2cmmn_1.rc[*issto - 1] = a2fpc_1.qp5 * ans / a2cmmn_1.rc[*issto - 1];
    a2cmmn_1.isigcr[*iissto - 1] = 0;
L6:
    if (a2cmmn_1.isigcr[*iissto - 1] != 1) {
	goto L200;
    }
    if ((d__1 = a2cmmn_1.rc[*issto - 1], abs(d__1)) == qqq45p) {
	goto L200;
    }
    iq = a2cmmn_1.ipontr[*iissto - 1];
    astor_(&a2cmmn_1.cansto[iq - 1], &c__8, &jjw, &c__4);
    if (jjw != 4 && jjw != 5 && jjw != 13 && jjw != 8) {
	goto L200;
    }
/* 300 CONTINUE */
    if (jjw != 8) {
...
/* ...         SPECIAL CASE FOR CONE */
    avdot_(&sn3[1], &a2cmmn_1.cansto[iq + 6], &ans);
    a2cmmn_1.rc[*issto - 1] = -d_sign(&a2cmmn_1.rc[*issto - 1], &ans);
    goto L200;
/* ....      CALCULATE RC FOR CYLINDER OR SPHERE USING CANONICAL FORM */
L100:
    avsto_(&a2cmmn_1.cansto[iq + 3], &a2cmmn_1.cc[*issto - 1]);
    avsub_(&sp3[1], &a2cmmn_1.cc[*issto - 1], a2cmmn_1.vtem);
    avdot_(&sn3[1], a2cmmn_1.vtem, &a2cmmn_1.rc[*issto - 1]);
L200:
    avmult_(&sn3[1], &a2cmmn_1.rc[*issto - 1], a2cmmn_1.vtem);
    avsub_(&sp3[1], a2cmmn_1.vtem, &a2cmmn_1.cc[*issto - 1]);
